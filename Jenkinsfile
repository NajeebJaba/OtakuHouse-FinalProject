...
pipeline {
    ...
    stages {
        ...
        stage('Run Tests') {
            steps {
                script {
                    bat "call ${VENV_DIR}\\Scripts\\activate && set PYTHONPATH=%PYTHONPATH%;${WORKSPACE} && ${PYTHON_PATH} -m pytest ${TEST_RUNNER} --html=${WORKSPACE}\\${HTML_REPORT_DIR}\\arcane_report.html"
                }
            }
        }
        stage('List Report') {
        ...
        stage('Install Dependencies') {
            steps {
                script {
                    // No need to use 'sh' in a Windows batch script block
                    bat "call ${VENV_DIR}\\Scripts\\pip install -r requirements.txt"
                }
            }
        }
